...
suppressMessages(packages(tm))
suppressMessages(packages(quanteda))
packages(ggplot2)
packages(qdap)
packages(wordcloud)
packages(stringr)
...
## setwd("C:/Users/dwaterson/Documents")
...
twitSamp <- suppressWarnings(smaller(readLines(file(textFiles[3],open = "r"), n=-1, skipNul = TRUE, encoding = "UTF-8")))

## Clean text samples after joining the three text source file samples
badWordsList <- read.csv("Bad Word List.csv", header = FALSE, stringsAsFactors = FALSE)
groupText <- c(blogSamp, newsSamp, twitSamp)

n <- removeNumbers(groupText)
n1 <- rm_stopwords(n, stopwords = tm::stopwords("english"), unlist = FALSE,
                   separate = FALSE, strip = TRUE, unique = FALSE,
                   char.keep = NULL, names = FALSE, ignore.case = TRUE,
                   apostrophe.remove = FALSE)
n2 <- rm_white(n1)
groupCorp <- VCorpus(VectorSource(n2))
groupCorpP <- tm_map(groupCorp, content_transformer(removePunctuation))
groupCorpPB <- tm_map(groupCorpP, removeWords, badWordsList$V1)

## create word cloud
## Reference: https://sites.google.com/site/miningtwitter/questions/talking-about/wordclouds/wordcloud1
tdm = TermDocumentMatrix(groupCorpPB)
...
# plot wordcloud
suppressWarnings(wordcloud(dm$word, dm$freq, random.order=FALSE, colors=brewer.pal(8, "Dark2")))
f <- findFreqTerms(tdm, 0, 31)
##f[1:20]

## Look for bigrams
BigramTokenizer <- function(x) NGramTokenizer(x, Weka_control(min = 2, max = 2))
tdmTok <- TermDocumentMatrix(groupCorpPB, control = list(tokenize = BigramTokenizer))
##inspect(tdmTok[225:255,1:20])
...
tdmTok3 <- TermDocumentMatrix(groupCorpPB, control = list(tokenize = TrigramTokenizer))
...
dmTri20 <- dmTri[1:20,]
## Plot trigrams
gTri <- ggplot(dmTri20, aes(reorder(word, -freq), freq))
gTri <- gTri + geom_bar(stat = "identity", fill = I("grey50"))
gTri <- gTri + labs(x = "Trigram", y = "Frequency")
gTri <- gTri + theme(axis.text.x = element_text(angle = 90, size = 12, hjust = 1))
gTri
```
